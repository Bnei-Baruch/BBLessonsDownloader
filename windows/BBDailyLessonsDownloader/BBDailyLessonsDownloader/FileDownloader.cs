...
namespace BBLessonsDwnldApp
{
    ...
    class FileDownloader : System.Object, IDisposable
    {
        ...
        public event EventHandler FileDownloadError;
        ...
        public string recentFileDownloaded;
        ...
        private void initizalize(Boolean supportsProgress)
        {
            ...
            this.StopWatchCyclesAmount = 10;
            this.DeleteCompletedFilesAfterCancel = false;
        }
        #endregion
        ...
        private void bgwDownloader_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                bgwDownloader.ReportProgress((Int32)InvokeType.FileDownloadFailedRaiser, ex);
        ...
        private void calculateFilesSize()
        {
            ...
            for (Int32 fileNr = 0; fileNr < this.Files.Count; fileNr++)
            {
                ...
                try
                {
                    webReq = (HttpWebRequest)WebRequest.Create(this.Files[fileNr].Path);
        ...
        private void downloadFile(Int32 fileNr)
        {
            ...
            else
            {
                ...
                exc = null;
                while (m_currentFileProgress < size && !bgwDownloader.CancellationPending && IsNetworkAvailable)
                {
                ...
                writer.Close();
                webResp.Close();
                if (!bgwDownloader.CancellationPending && IsNetworkAvailable && exc == null) { recentFileDownloaded = file.Name; fireEventFromBgw(Event.FileDownloadSucceeded); }
                if (!IsNetworkAvailable || exc != null)
                {
                    bgwDownloader.ReportProgress((Int32)InvokeType.FileDownloadFailedRaiser, exc);
                ...
                case InvokeType.EventRaiser:
                    switch ((Event)e.UserState)
                    {
                        ...
                        case Event.ProgressChanged:
                            ...
                            break;
                        case Event.FileDownloadError:
                            if (FileDownloadError != null) { this.FileDownloadError(this, new EventArgs()); }
                            break;
                    }
                    break;
